...
function addRowToReservationTable(data, key) {
  ...
  newRow.insertCell().textContent = formatPrice(data.montantPaye || 0);
  newRow.insertCell().textContent = formatPrice(data.price);
...
function updateRowInTable(data, key, tableId = 'dataTable') {
  ...
  if (rowToUpdate) {
    ...
    const imageCellIndex = tableId === 'reservationTable' ? 6 : 5;
    if (data.image) {
    ...
    if (tableId === 'reservationTable' && rowToUpdate.cells.length === 8) {
      const actionsCell = rowToUpdate.cells[7];
